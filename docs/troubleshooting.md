...
- Confirm upstream/ports: nginx must proxy `homeassistant:8123`; only nginx publishes 80/443; LAN-only access keeps UFW to LAN on 22/8123.
- For local curls, force Host/SNI: `curl --resolve home.example.net:443:127.0.0.1 https://home.example.net/`.
...
- Wrong Host/SNI yields 403; always send the real hostname, even for local curls.
