...
- **`ha_net`** (e.g. `172.21.0.0/16`)
  ...
  - Only nginx publishes 80/443 to the host.
...
Config files under `/etc/ufw/` are host-specific and are not tracked in the repo; apply rules manually on the target host.
...
- **LAN:**
  - nginx (80/443)
  - HA (8123) - direct web UI if desired
  - SSH (22)

- **Internal-only:**
  - MQTT 8883 (TLS)
  - Frigate API / UI (if not published)
