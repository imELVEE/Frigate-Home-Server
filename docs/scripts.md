...
- The tracked schedules are `/etc/cron.weekly/trivy-scan` and `/etc/cron.weekly/lynis-audit`.
...
- On failure, sends an email and publishes an MQTT alert to `server/<hostname>/health` from inside the Mosquitto container.
- The tracked timer is `server-healthcheck.timer`, which runs every 30 minutes.
...
- The tracked schedule is `/etc/cron.d/disk-healthcheck`, which runs every 4 hours.

---

### `server_make_snapshot.sh`

- Creates versioned config snapshots containing `/etc`, `~/ha`, and `~/scripts`, plus package and Docker metadata.
...
### `acme_renew.sh` and `notify_cert_failure.sh`

- `acme_renew.sh` runs `acme.sh` in Docker using `/etc/acme/dynu.env` and deploys renewed certs into `~/ha/nginx/ssl/`.
- If renewal fails, it calls `notify_cert_failure.sh`.
- `notify_cert_failure.sh` sends a concise failure email through `notify_email.py`.
- The tracked timer is `acme-renew.timer`, which runs hourly.
...
- Triggered from `/etc/logrotate.d/ha-modsec` after the WAF audit log is rotated.
...
- `acme_renew.sh` uses `/etc/acme/dynu.env` because its DNS credentials belong to the root-managed ACME flow, not the user-owned secrets folder.
...
- Tracked scheduled entrypoints:
  - `server-healthcheck.timer` runs `server_healthcheck.sh` every 30 minutes.
  - `acme-renew.timer` runs `acme_renew.sh` hourly.
...
- `server_make_snapshot.sh` is a rebuild-oriented config snapshot, not a full home-directory or media backup.
